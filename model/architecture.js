...
module.exports = (mainWindow) => {
    ...
    ipc.on('getArchitecture', function (e, data) {
        ...
        db.architecture.find(value).sort({ planet: 1 }).skip(data.start).limit(data.size).exec(function (err, docs) {
    ...
    ipc.on('deleteArchitecture', function (e, data) {
        for (let d of data) {
            db.architecture.remove({ _id: d['_id'] }, {}, function (err, numRemoved) {
                db.floor.find({ architectureId: d['_id'] }, function (errf, docsf) {
                    for (let floor of docsf) {
                        db.floor.remove({ _id: floor['_id'] }, {}, function (err, numRemoved) {
                            db.room.find({ floorId: floor['_id'] }, function (errr, docsr) {
